...
<div class="admin-card mb-3">
    <form method="GET" action="{{ route('admin.skills.index') }}" class="d-flex gap-3 flex-wrap align-items-end">
        <div class="flex-grow-1">
            <label class="form-label-admin">Search</label>
            <div class="search-wrap">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search skills..." class="form-input-admin search-input">
            </div>
        </div>
        ...
        <button type="submit" class="btn-admin btn-outline-admin" style="height:42px;">
            <i class="fa-solid fa-filter"></i> Filter
        </button>
        <a href="{{ route('admin.skills.index') }}" class="btn-admin btn-outline-admin" style="height:42px;">Reset</a>
    </form>
</div>
...
<div class="admin-card">
    <table class="admin-table">
        ...
        <tbody>
            ...
            <tr>
                <td style="color:var(--muted); font-size:0.8rem;">{{ $skill->id }}</td>
                <td><span style="font-weight:600;">{{ $skill->skill_name }}</span></td>
                <td>
                    <span class="badge-pill {{ $skill->skill_type === 'technical' ? 'badge-technical' : 'badge-soft' }}">
                        {{ $skill->skill_type }}
                    </span>
                </td>
                <td style="font-size:0.8rem; color:var(--muted); max-width:280px;">
                    {{ Str::limit($skill->keywords, 60) }}
                </td>
                <td>
                    <div class="d-flex gap-2">
                        <a href="{{ route('admin.skills.edit', $skill) }}" class="btn-admin btn-outline-admin" style="padding:5px 12px; font-size:0.78rem;">
