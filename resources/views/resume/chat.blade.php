...
/* ── Download block ───────────────────────────────────────── */
// ✅ FIX: Save the full transcript to the server FIRST, then submit
//         the download form so the PDF includes all user messages.
function showDownload() {
    ...
    wrap.id = 'dl-block';
    wrap.innerHTML = `
        <button type="button" class="btn btn-glow" id="dl-btn">
    ...
    document.getElementById('dl-btn').addEventListener('click', function () {
        ...
        // Step 1: save full transcript to server
        fetch(SAVE_URL, {
            method:  'POST',
            headers: {
        ...
        .then(() => {
            // Step 2: submit a hidden form to trigger the PDF download
            ...
            setTimeout(() => {
                ...
                btn.innerHTML = '<i class="fa-solid fa-file-pdf me-2"></i>Download My Report';
            }, 3000);
        })
        .catch(() => {
            // Even if save fails, still allow download
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = DOWNLOAD_URL;
            const csrf = document.createElement('input');
...
const RULES = [
    ...
    { keys: ['salary','pay','earn','income','wage','money'],
      reply: () => `Salaries for <strong>${ucwords(careerName)}</strong> vary a lot by location, company size, and experience. I'd recommend checking Glassdoor or LinkedIn Salary for current numbers in your area.`,
      next: null },
    ...
    { keys: ['interview','prep','question','tips'],
      reply: () => `For <strong>${ucwords(careerName)}</strong> interviews, expect questions around your strongest skills, how you handle challenges, and technical problem-solving. Practice the STAR method (Situation → Task → Action → Result) for behavioural questions.`,
      next: null },
